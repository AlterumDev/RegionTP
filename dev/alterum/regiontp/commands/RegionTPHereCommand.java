	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (player.hasPermission(Configuration.tphere_permission) || player.hasPermission(Configuration.admin_permission)) {
			if (args == null || args.length < 1 || args.length == 0) {
				...
				Location hereLoc = new Location(player.getWorld(), player.getLocation().getX(), player.getLocation().getY(), player.getLocation().getZ());
				...
				if (regions != null) {
					if (regions.hasRegion(originName)) {
						...
						if (playersTotal > 0) {
							player.sendMessage(Utils.format(Configuration.tp_success.replace("{REGION}", originName).replace("{PREFIX}", prefix).replace("{PLAYERS}", Integer.toString(playersTotal))));
						} else {
